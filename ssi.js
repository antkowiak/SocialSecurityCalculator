...
const averageWageLastYear = Math.max(...Object.keys(wageIndex));
...
const AWI_Factors = Object.entries(wageIndex).reduce(( acc, [i, val], ) => (
    (acc[i] = 1 + (wageIndex[averageWageLastYear] - val) / val) && acc )
, {});

const AdjustedEarnings = Object.entries(earnings).reduce(( acc, [i, val], ) => (
    (acc[i] = val * (AWI_Factors[i] || futureYearsFactor)) && acc )
, {});
